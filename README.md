<div align="center">

# 🩵 Hi, I'm EXIT_SUCCESS! 🩷

<p align="center">
  <img src="./banner.png" width="800">
</p>

### `C++ Developer` · `Backend` · `Telegram Bots`

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,14,20&height=120&section=header&text=Welcome!&fontSize=40&fontColor=ffffff&animation=fadeIn&fontAlignY=40" width="100%"/>

[![C++](https://img.shields.io/badge/C%2B%2B20-5B9BD5?style=for-the-badge\&logo=cplusplus\&logoColor=white)](https://isocpp.org/)
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-6FA8DC?style=for-the-badge\&logo=postgresql\&logoColor=white)](https://www.postgresql.org/)
[![Git](https://img.shields.io/badge/Git-F28B9C?style=for-the-badge\&logo=git\&logoColor=white)](https://git-scm.com/)
[![Telegram](https://img.shields.io/badge/Telegram-78C7E8?style=for-the-badge\&logo=telegram\&logoColor=white)](https://telegram.org/)

</div>

---

<div align="center">

## 🌸 `> whoami`

**C++ developer focused on backend development**

`C++20` • `PostgreSQL` • `TgBot` • `Git` • `CMake`

</div>

---

## 🩵 Tech Stack

<table align="center">
<tr>
<td align="center" width="33%">

### 💻 Languages

<img src="https://skillicons.dev/icons?i=cpp,python&theme=light"/>

</td>

<td align="center" width="33%">

### 🗄️ Backend

<img src="https://skillicons.dev/icons?i=postgresql&theme=light"/>

<br>

`TgBot` · `REST API`

</td>

<td align="center" width="33%">

### 🔧 Tools

<img src="https://skillicons.dev/icons?i=git,cmake,linux,vscode&theme=light"/>

</td>
</tr>
</table>

---

# 📊 GitHub Statistics

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=DARK4922&show_icons=true&hide_border=true&bg_color=ffffff&title_color=5B9BD5&icon_color=F28B9C&text_color=4A5568&ring_color=78C7E8"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=DARK4922&layout=compact&hide_border=true&bg_color=ffffff&title_color=5B9BD5&text_color=4A5568"/>

</div>

<br>

<div align="center">

<img src="https://streak-stats.demolab.com?user=DARK4922&hide_border=true&background=FFFFFF&ring=78C7E8&fire=F28B9C&currStreakLabel=5B9BD5&sideLabels=5B9BD5&currStreakNum=4A5568&sideNums=4A5568&dates=8A94A6"/>

</div>

---

# 🩷 Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=DARK4922&bg_color=ffffff&color=5B9BD5&line=78C7E8&point=F28B9C&area=true&hide_border=true&custom_title=Vanya's%20Contribution%20Graph"/>

</div>

---

# 🚀 Featured Project

<div align="center">

## 🤖 Anime Sticker Bot

**C++20 · TgBot · PostgreSQL · nlohmann/json**

</div>

Telegram bot for managing anime sticker packs with moderation,
applications and a rating system.

```text
🎴 Sticker packs       ⭐ Ratings
🗂️ Categories          🛡️ Moderation
📨 Applications        🗄️ PostgreSQL
👤 Users               ⚡ C++20
```

<div align="center">

[![View Repository](https://img.shields.io/badge/VIEW%20REPOSITORY-78C7E8?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/DARK4922/YOUR_REPOSITORY)

</div>

---

# 📌 What I'm Working On

```text
C++20
 ├── ⚙️ Backend development
 ├── 🧵 Multithreading
 ├── 🗄️ PostgreSQL
 ├── 🏗️ Software architecture
 └── 🚀 Performance
```

---

# 🌱 Currently Learning

<div align="center">

`System Design` · `Advanced C++` · `PostgreSQL`
`Concurrency` · `Clean Architecture` · `Algorithms`

</div>

---

<div align="center">

## 🩵 A little more about me 🩷

💻 Programming since **12**
🧩 Can solve **3×3 · 4×4 · 5×5 · 6×6 · 7×7**
🎴 Anime enjoyer
🎧 Music while coding
☕ Probably debugging something

</div>

---

<div align="center">

### 🌸 Thanks for visiting! 🌸

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=18&duration=3000&pause=1000&color=5B9BD5&center=true&vCenter=true&width=500&lines=Code+%E2%86%92+Compile+%E2%86%92+Debug+%E2%86%92+Repeat;Keep+learning.+Keep+building.+%F0%9F%8C%B8" />

<br>

🩵 **C++**   🩷 **Backend**   🩵 **Anime**   🩷 **Code**

</div>
